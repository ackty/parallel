/*
 * C code using OpenMP. Compiled with gcc -Wall -std=c99 -fopenmp, where GCC 4.2 or later is required. 
 * Note that the code finds the largest first prime factor, but does not return the factor list: 
 * it's just a matter of repeating the prime factor test, which adds clutter but does not make the code any more interesting. 
 * For that matter, the code uses the dumbest prime factoring method, and doesn't even test if the numbers can be divided by 2.
 */
#include <stdio.h>
#include <omp.h>
